...
extern uint8_t BlueBtnPressed;

AIRBEAN_IO_STATUS ioStat = {
	{DRYIN_ST_Z, DRYIN_ST_Z},
	{RELAY_ST_OFF, RELAY_ST_OFF},
	{DIP_ST_Z, DIP_ST_Z},
	false,
	false
};
...
/** @defgroup STM324xG_EVAL_LOW_LEVEL_Private_Functions
  * @{
  */ 
void abEval_IOInit(void)
{
	/*RalayInit*/
	GPIO_InitTypeDef  GPIO_InitStructure;
	RCC_AHB1PeriphClockCmd(RELAY1_CLK|RELAY2_CLK|DRYIN1_CLK|DRYIN2_CLK, ENABLE);
	...
	GPIO_InitStructure.GPIO_Pin = DRYIN1_PIN ;
	GPIO_Init(DRYIN1_PORT, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = DRYIN2_PIN ;
	GPIO_Init(DRYIN2_PORT, &GPIO_InitStructure);
	
	/*DIP SW*/
	RCC_AHB1PeriphClockCmd(DIP_SW1_CLK|DIP_SW2_CLK, ENABLE);
	...
	
	{
		...
		SYSCFG_EXTILineConfig(DRYIN1_EXTI_PORT_SOURCE, DRYIN1_EXTI_PIN_SOURCE);

		/* Configure Button EXTI line */
		EXTI_InitStructure.EXTI_Line = DRYIN1_EXTI_LINE;
		...
		/* Connect Button EXTI Line to Button GPIO Pin */
		SYSCFG_EXTILineConfig(DRYIN2_EXTI_PORT_SOURCE, DRYIN2_EXTI_PIN_SOURCE);

		/* Configure Button EXTI line */
		EXTI_InitStructure.EXTI_Line = DRYIN2_EXTI_LINE;
		EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
		...
		NVIC_InitStructure.NVIC_IRQChannel = DRYIN1_EXTI_IRQn;
		...
		/* Enable and set Button EXTI Interrupt to the lowest priority */
		NVIC_InitStructure.NVIC_IRQChannel = DRYIN2_EXTI_IRQn;
	...
	ioStat.relay[0] = RELAY_ST_OFF;
	ioStat.relay[1] = RELAY_ST_OFF;
	
	ioStat.dip[0] = (DIP_STATUS)GPIO_ReadInputDataBit(DIP_SW1_PORT, DIP_SW1_PIN);
	ioStat.dip[1] = (DIP_STATUS)GPIO_ReadInputDataBit(DIP_SW2_PORT, DIP_SW2_PIN);
			
	ioStat.needAnnouce = true;
	ioStat.needUpdate = true;
}
void DRYIN1_IRQHandler()
{
	if(EXTI_GetITStatus(DRYIN1_EXTI_LINE) == SET)
	{
		ioStat.needAnnouce = true;
		/* Clear the EXTI line pending bit */
		EXTI_ClearITPendingBit(DRYIN1_EXTI_LINE);
	}
	if(EXTI_GetITStatus(BLUE_BTN_EXTI_LINE) == SET)
	{
		BlueBtnPressed = 0x01;
		/* Clear the EXTI line pending bit */
		EXTI_ClearITPendingBit(BLUE_BTN_EXTI_LINE);
	}
}
void DRYIN2_IRQHandler()
{
	ioStat.needAnnouce = true;
	/* Clear the EXTI line pending bit */
	EXTI_ClearITPendingBit(DRYIN2_EXTI_LINE);
}
void abEval_RelaySet(Relay_TypeDef idx, RELAY_STATUS stat)
{
	if(idx < nRelay)
	{
		ioStat.relay[idx] = stat;
		ioStat.needUpdate = true;
	}
}
void abEval_RelayToggle(Relay_TypeDef idx)
{
	if(idx < nRelay)
	{
		ioStat.relay[idx] = (ioStat.relay[idx]==RELAY_ST_OFF)?RELAY_ST_ON:RELAY_ST_OFF;
		ioStat.needUpdate = true;
	}
}
char reportIO_Zb[32];
void abEval_IOProcess(void)
{
	if(ioStat.needAnnouce)
	{
		ioStat.needAnnouce = false;
		ioStat.dryin[0] = (DRYIN_STATUS)DRYIN1_STATUS();
		ioStat.dryin[1] = (DRYIN_STATUS)DRYIN2_STATUS();
		
		if(ioStat.dryin[DRYIN1] == DRYIN_ST_Z) {
			LEDx_ON(LED3);
		}
		else {
			LEDx_OFF(LED3);
		...
		if(ioStat.dryin[DRYIN2] == DRYIN_ST_Z) {
			LEDx_ON(LED4);
		}
		else {
			LEDx_OFF(LED4);
		...
		/* "d00" */
		sprintf(reportIO_Zb, "d%d%d\0", ioStat.dryin[0], ioStat.dryin[1]);
		dgZigbeeApiSendString(0, reportIO_Zb);
	}
	
	if(ioStat.needUpdate)
	{
		ioStat.needUpdate = false;
		RELAY1_SET(ioStat.relay[0]);
		RELAY2_SET(ioStat.relay[1]);

		if(ioStat.relay[RELAY1] == RELAY_ST_ON) {
			LEDx_ON(LED1);
		}
		else {
			LEDx_OFF(LED1);
		...
		if(ioStat.relay[RELAY2] == RELAY_ST_ON) {
			LEDx_ON(LED2);
		}
		else {
			LEDx_OFF(LED2);
		...
		/* "d00" */
		sprintf(reportIO_Zb, "r%d%d\0", ioStat.relay[0], ioStat.relay[1]);	
		dgZigbeeApiSendString(0, reportIO_Zb);
	}
	
#ifdef DIP_SW1_PIN
	{
		static uint16_t dipTick = 0;
		if((getVar_Time_Rec_Base()-dipTick)>100)	/*1000 ms*/
		{
			if(GPIO_ReadInputDataBit(DIP_SW1_PORT, DIP_SW1_PIN) != ioStat.dip[0])
			{
				abEval_RelayToggle(RELAY1);
				ioStat.dip[0] = GPIO_ReadInputDataBit(DIP_SW1_PORT, DIP_SW1_PIN);
			}
			
			if(GPIO_ReadInputDataBit(DIP_SW2_PORT, DIP_SW2_PIN) != ioStat.dip[1])
			{
				abEval_RelayToggle(RELAY2);
				ioStat.dip[1] = GPIO_ReadInputDataBit(DIP_SW2_PORT, DIP_SW2_PIN);
			}
			
			dipTick = getVar_Time_Rec_Base();
		}
	}
#endif
}

uint8_t abEval_getDryin(Dryin_TypeDef idx)
{
	return (uint8_t)ioStat.dryin[idx];
}
uint8_t abEval_getRelay(Relay_TypeDef idx)
{
	return (uint8_t)ioStat.relay[idx];
}
