...
typedef struct {
	DRYIN_STATUS dryin[nDryin];
	RELAY_STATUS relay[nRelay];
	DIP_STATUS dip[nDip];
	bool needAnnouce;
	bool needUpdate;
}AIRBEAN_IO_STATUS;
...
/** 
  ...
  */ 
#if !defined (USE_STM324xAB_EVAL)
 #define USE_STM324xAB_EVAL
#endif
...
#define LEDx_ON(x)		{GPIO_PORT[x]->BSRRL = GPIO_PIN[x];}
#define LEDx_OFF(x)		{GPIO_PORT[x]->BSRRH = GPIO_PIN[x];}

#define DRYIN1_STATUS()	GPIO_ReadInputDataBit(DRYIN1_PORT, DRYIN1_PIN)
#define DRYIN2_STATUS()	GPIO_ReadInputDataBit(DRYIN2_PORT, DRYIN2_PIN)

#define RELAY1_STATUS()	GPIO_ReadOutputDataBit(RELAY1_PORT, RELAY1_PIN)
#define RELAY2_STATUS()	GPIO_ReadOutputDataBit(RELAY2_PORT, RELAY2_PIN)

#define RELAY1_SET(x)	GPIO_WriteBit(RELAY1_PORT, RELAY1_PIN, (BitAction)x)
#define RELAY1_TOGGLE()	GPIO_ToggleBits(RELAY1_PORT, RELAY1_PIN)
#define RELAY2_SET(x)	GPIO_WriteBit(RELAY2_PORT, RELAY2_PIN, (BitAction)x)
